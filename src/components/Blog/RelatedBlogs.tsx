  ...
  useEffect(() => {
    const fetchRelatedBlogs = async () => {
      try {
        ...
        const response = await getBlogsListingPageClient(1, ITEMS_PER_PAGE, industryFilters);
    ...
    <div className="mt-12">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {relatedBlogs.map((blog: any) => (
          <LocalizedLink href={`/blogs/${blog.attributes.slug}`} key={blog.id}>
            <BlogItem
              title={blog.attributes.title?.length > 60 ? blog.attributes.title.substring(0, 50) + '...' : blog.attributes.title}
              thumbnailImage={blog.attributes.thumbnailImage}
              shortDescription={blog.attributes.shortDescription?.substring(0, 80)+"..."}
              date={new Intl.DateTimeFormat('en-IN', {
                day: '2-digit',
                month: 'short',
                year: 'numeric',
              }).format(new Date(blog.attributes.oldPublishedAt || blog.attributes.publishedAt))}
