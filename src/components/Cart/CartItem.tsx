import reportPlaceholder from '@/assets/img/sampleResearch.png';
import { FC } from 'react';
import { FaCircleMinus, FaCirclePlus } from 'react-icons/fa6';
import { IoCloseCircle } from 'react-icons/io5';

interface CartItemProps {
   name: string;
   price: number;
   img?: string;
   quantity: number;
}

const CartItem: FC<CartItemProps> = ({
   name,
   price,
   img = reportPlaceholder.src,
   quantity,
}) => {
   return (
      <div className='flex justify-between border-b border-s-500 pb-4 pt-4 font-semibold first-of-type:pt-0 last-of-type:border-0 last-of-type:pb-0 md:text-lg'>
         <div className='flex w-1/2 items-center gap-3'>
            <span className='cursor-pointer text-xl text-gray-500'>
               <IoCloseCircle />
            </span>
            <img
               src={img}
               alt=''
               className='h-16 w-16 rounded-sm border border-s-300'
            />
            <p className='text-blue-3'>{name}</p>
         </div>
         <div className='flex w-1/2 items-center gap-3 md:gap-6'>
            <div className='w-1/3'>
               <p className='text-blue-3'>${price}</p>
            </div>
            <div className='w-1/3'>
               <div className='flex w-max gap-4 rounded-md border border-s-300 px-4 py-2'>
                  <button
                     className='text-xl text-blue-2 disabled:text-gray-300'
                     disabled={quantity === 1}
                  >
                     <FaCircleMinus />
                  </button>
                  <p className='text-sm font-normal text-gray-700'>
                     {quantity}
                  </p>
                  <button className='text-xl text-blue-2 disabled:text-gray-300'>
                     <FaCirclePlus />
                  </button>
               </div>
            </div>

            <div className='w-1/3'>
               <p className='text-blue-3'>${price * quantity}</p>
            </div>
         </div>
      </div>
   );
};

export default CartItem;
